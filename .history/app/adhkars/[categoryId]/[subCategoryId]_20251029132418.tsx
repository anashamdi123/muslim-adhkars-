...
import Animated, {
  ...
  useSharedValue,
    ...
    export default function DhikrDetailScreen() {
      ...
      const renderDhikrCard = ({ item, index }: { item: any; index: number }) => {
        ...
        const progress = total > 0 ? (total - count) / total : 1; // 0..1
  ...
  function ProgressCircle({
    ...
    useEffect(() => {
      progressValue.value = withTiming(progress, { duration: 350 });
    }, [progress]);

    const rightRotationStyle = useAnimatedStyle(() => {
      const rotation = interpolate(progressValue.value, [0, 0.5], [0, 180]);
      return { transform: [{ rotate: `${rotation}deg` }] };
    });

    const leftRotationStyle = useAnimatedStyle(() => {
      const rotation = interpolate(progressValue.value, [0.5, 1], [0, 180]);
      return { transform: [{ rotate: `${rotation}deg` }] };
    });

    const radius = size / 2;
    const half = radius; // half width container
    ...
    return (
      ...
      >
        ...
        {/* Left half progress (activates after 50%) */}
        <View
          style={{
            position: 'absolute',
            width: half,
